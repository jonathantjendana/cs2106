    ...
    while (fgets(buffer, sizeof(buffer), stdin) != NULL) {
        ...
        for (int i = 0; i < commandNum; ++i) {
            if (!strcmp(commandsTokens[i][0], "quit")) {
                ...
                fflush(stdout);
                return 0;
            } else if(!strcmp(commandsTokens[i][0], "set")) {
               setenv(commandsTokens[i][1], commandsTokens[i][2], 1);
               freeCommandsArray(commandsTokens, commandNum, commandsTokenNum);
               isSet = true;
               break;
            } else if(!strcmp(commandsTokens[i][0], "unset")) {
                unsetenv(commandsTokens[i][1]+1);
                freeCommandsArray(commandsTokens, commandNum, commandsTokenNum);
                isSet = true;
                break;
            ...
            for (int j = 0; j < commandsTokenNum[i]; ++j) {
                if (commandsTokens[i][j][0] == '$') {
                    char *tmp = getenv(commandsTokens[i][j]+1);
                    if (tmp) {
                        strcpy(commandsTokens[i][j], tmp);
                    } else {
                        commandsTokens[i][j][0] = '\0';
                    }
                }
            }
        }
